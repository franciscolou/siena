...
API_KEY = 'oP8n9vE7pQ4L6rT1kY2cX3wM0zB5fH9dN7gV4aU8jP2qS3xW6lR1mC8oJ3'

# Initialize a connection pool
def init_connection_pool(size):
    pool = Queue(maxsize=size)
    for _ in range(size):
        conn = sqlite3.connect('api_pancada/database.db', check_same_thread=False, timeout=10)  # Replace with your actual database connection
        pool.put(conn)
    return pool
...
def calculate_changes(last_members, old_members):
    ...
    if "Kikolou" in old_members_ids:
        print("Kikolou in old_members")
    if "Kikolou" in last_members_ids:
        print("Kikolou in last_members")
    members_that_left_ids = old_members_ids - last_members_ids
    new_members_ids = last_members_ids - old_members_ids

    print(new_members_ids)
    ...
    print("new_members_dicts: ", new_members_dicts)
...
def update_database(conn, group_name, changes, last_members, old_members):
    ...
    if changes:
        ...
        for change in changes:
            ...
            print(f"Inserted change: {change}")
        ...
        print(f"Inserted admlog: {changes[0]['timestamp']}, {json.dumps(adms_online)}")

    # Update members table
    cursor.execute(f"DELETE FROM {group_name}")
    print(f"Deleted all rows from {group_name}")
    ...
    print("consulta depois de teoricamente adicionar: ", changes_result)
    cursor.execute(f"SELECT * FROM {group_name}_admlog")
    print("admlog: ", cursor.fetchall())
    ...
    conn.commit()
    cursor.execute(f"SELECT * FROM {group_name}_changes")
    print("consulta depois de teoricamente adicionar: ", cursor.fetchall())
...
def get_all_users():
    ...
    # Fetch all users' names and roles from the database
    cursor.execute("SELECT name, role FROM credentials")
    rows = cursor.fetchall()
    return_connection(connection_pool, conn)
    ...
    return Response(json.dumps(users, sort_keys=False), mimetype='application/json')
